...
function drawHeader(doc, { invoice, business }) {
  ...
  // Caja de la letra C solamente arriba. No dibujamos rect醤gulo vac韔 debajo.
  ...
  labelValue(doc, 'Condici髇 frente al IVA:', business.taxCondition, x + 10, y + 128, x + 122, {
...
function drawFooter(doc, { invoice, qrBuffer }) {
  ...
  doc.fillColor('black').font('Helvetica').fontSize(5.8).text('AGENCIA DE RECAUDACI脫N\nY CONTROL ADUANERO', x + 96, y + 32, {
    width: 100
  });

  doc.font('Helvetica-Bold').fontSize(8.5).text('Comprobante Autorizado', x + 95, y + 59);
  doc.font('Helvetica-Oblique').fontSize(6.2).text(
    'Esta Agencia no se responsabiliza por los datos ingresados en el detalle de la operaci贸n',
    x + 95,
    y + 78,
    { width: 250 }
  );

  doc.font('Helvetica-Bold').fontSize(9).text('P谩g. 1/1', x + 250, y + 18, {
    width: 80,
    align: 'center'
  });

  labelValue(doc, 'CAE N掳:', invoice.cae || '', x + 395, y + 18, x + 448, { width: 95, align: 'left', fontSize: 9 });
