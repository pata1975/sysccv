...
function drawHeader(doc, { invoice, business }) {
  ...
  labelValue(doc, 'Razón Social:', business.legalName, x + 10, y + 72, x + 78, { width: leftW - 88 });
  labelValue(doc, 'Domicilio Comercial:', business.address, x + 10, y + 100, x + 106, { width: leftW - 116 });
  labelValue(doc, 'Condición frente al IVA:', business.taxCondition, x + 10, y + 128, x + 122, { width: leftW - 132 });
  ...
  labelValue(doc, 'Fecha de Emisión:', formatDisplayDate(invoice.issuedAt), rightX, y + 84, rightX + 92, { width: 120 });
...
function drawBuyerBox(doc, { order }) {
  ...
  labelValue(doc, 'Apellido y Nombre / Razón Social:', order.buyerName || 'Consumidor Final', x + 205, y + 9, x + 370, { width: 170 });
  labelValue(doc, 'Condición frente al IVA:', order.buyerTaxpayerType || 'Consumidor Final', x + 8, y + 34, x + 120, { width: 190 });
  labelValue(doc, 'Domicilio:', buildAddressLine(order.buyerAddress), x + 315, y + 34, x + 370, { width: 170 });
  labelValue(doc, 'Condición de venta:', process.env.INVOICE_SALES_CONDITION || 'Otros medios de pago electrónico', x + 8, y + 56, x + 110, { width: 250 });
}
...
function drawItemsTable(doc, { order }) {
  ...
  const headerH = 20;
  const columns = [
    { label: 'Código', x, width: 40, align: 'center' },
...
function drawFooter(doc, { invoice, qrBuffer }) {
  ...
  doc.fillColor('black').font('Helvetica').fontSize(5.8).text('AGENCIA DE RECAUDACIÓN\nY CONTROL ADUANERO', x + 96, y + 32, {
    width: 100
  });
  ...
  doc.font('Helvetica-Bold').fontSize(9).text('Pág. 1/1', x + 250, y + 18, {
    width: 80,
    align: 'center'
  });

  labelValue(doc, 'CAE N°:', invoice.cae || '', x + 395, y + 18, x + 448, { width: 95, align: 'left', fontSize: 9 });
